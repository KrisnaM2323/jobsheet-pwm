...
if(isset($_POST['import'])){
  ...
  while ($i <= count($all_data)) {    
    $nama_pelajaran = $all_data[$i]['A'];
    $keterangan = $all_data[$i]['B'];

    $sql = "INSERT INTO tb_pelajaran SET nama_pelajaran='$nama_pelajaran', keterangan='$keterangan'";
  ...
  //TODO echo $all_data[3]['A'];

  // echo $sql;

  unlink($target_file);
  // echo "<script>window.alert('')</script>";
  // echo "<script>window.location='../tabel_pelajaran.php'</script>";
header("location: ../tabel_pelajaran.php?pesan=import_excel");
